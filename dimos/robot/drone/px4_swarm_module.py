...
MOTION_REFERENCE_MAX_LEAD_M = 8.0
MOTION_FEEDFORWARD_STOP_RADIUS_M = 3.0
MOTION_FEEDFORWARD_FULL_RADIUS_M = 10.0
...
class PX4SwarmModule(Module):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.origin_positions_enu = origin_positions_enu or _default_origins(len(self.connection_strings))
        self.drone_keys = drone_keys or [f"x500_{index}" for index in range(len(self.connection_strings))]
    ...
    def start(self) -> None:
        ...
        if len(self.connection_strings) != len(self.origin_positions_enu):
            raise ValueError("connection_strings and origin_positions_enu must have the same length")
    ...
    def _ensure_velocity_control(self) -> str | None:
        ...
        setup_failure = self._run_with_zero_velocity_stream(self._enter_offboard_and_arm_all)
    ...
    def _velocity_control_state_failure(self, require_airborne: bool) -> str | None:
        ...
        for drone in self._drones:
            ...
            if not snapshot.armed:
                ...
                continue
            if require_airborne and snapshot.position_enu[2] < AIRBORNE_ALTITUDE_M:
                failures.append(
            ...
            if not drone.is_mode("OFFBOARD"):
                failures.append(self._drone_failure_detail(drone, f"is in {snapshot.mode}, not OFFBOARD"))
        if failures:
    ...
    def _airborne_state_failure(self) -> str | None:
        ...
        for drone in self._drones:
            ...
            if not snapshot.has_local_position:
                ...
                failures.append(self._drone_failure_detail(drone, "is disarmed"))
            elif snapshot.position_enu[2] < AIRBORNE_ALTITUDE_M:
                failures.append(
                    self._drone_failure_detail(
                        drone,
                        f"is not airborne altitude={snapshot.position_enu[2]:.1f}m",
        ...
        if failures:
            return "Failed: hover requires airborne drones; use takeoff_swarm first: " + "; ".join(failures)
        return None

    def _enter_offboard_and_arm_all(self) -> str | None:
        time.sleep(2.0)
        ...
        for drone in self._drones:
            if not drone.set_mode("OFFBOARD"):
                offboard_failures.append(self._drone_failure_detail(drone, "rejected OFFBOARD mode"))
    ...
    def _reset_takeoff_modes_before_arm(self) -> str | None:
        ...
        for drone in self._drones:
            ...
            if near_ground and reset_needed and not drone.set_mode("LOITER") and snapshot.armed:
                failures.append(self._drone_failure_detail(drone, "could not reset mode to LOITER before takeoff"))
        if failures:
            return "Failed: " + "; ".join(failures)
        return None

    def _native_takeoff_until_airborne(self) -> str | None:
        target_altitude = max(self.takeoff_altitude_m, 1.5)
        min_airborne_altitude = min(0.7, target_altitude * 0.35)
        positions = self._positions_enu()
        low_drones = [
            drone
        ...
        deadline = time.time() + max(45.0, target_altitude * 8.0)
        next_takeoff_request_s = time.time() + 2.0
        while time.time() < deadline:
            positions = np.asarray(
                [drone.update(timeout=0.03).position_enu for drone in self._drones],
                dtype=float,
            )
            if all(position[2] >= min_airborne_altitude for position in positions):
                return None
            now = time.time()
    ...
    def _wait_until_mode(self, mode: str, timeout_s: float) -> str | None:
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.05)
        failures = [self._drone_failure_detail(drone, f"did not enter {mode}") for drone in not_in_mode]
        return "Failed: " + "; ".join(failures)
    ...
    def _arm_all(self, timeout_s: float, force: bool = False) -> str | None:
        ...
        if not not_armed:
            return None
        if not_armed:
            failures = [self._drone_failure_detail(drone, "did not report armed") for drone in not_armed]
            return "Failed: " + "; ".join(failures)
        return None
    ...
    def _wait_until_landed(self, timeout_s: float) -> bool:
        ...
        while time.time() < deadline:
            ...
            all_grounded = bool(snapshots) and all(
                snapshot.position_enu[2] <= AIRBORNE_ALTITUDE_M + 0.3
    ...
    def _wait_for_position_stream(self, timeout_s: float) -> str | None:
        ...
        details = [
            self._drone_failure_detail(drone, "no LOCAL_POSITION_NED stream")
            for drone in missing
        ]
        return "Failed: " + "; ".join(details)
    ...
    def _cleanup_control_setup_failure(self) -> None:
        ...
        if not self._drones:
            return
        zeros = np.zeros((len(self._drones), 3), dtype=float)
        self._command_all_for(zeros, seconds=0.5)
        for drone in self._drones:
            snapshot = drone.update(timeout=0.02)
    ...
    def _send_all_position_velocity_setpoints(
        ...
        for drone, position, velocity in zip(self._drones, positions_enu, velocities_enu, strict=True):
            drone.send_position_velocity_enu(position, velocity)

    def _advance_reference_points(
        self,
        reference_points_enu: np.ndarray[Any, Any],
        positions_enu: np.ndarray[Any, Any],
        desired_points_enu: np.ndarray[Any, Any],
        velocities_enu: np.ndarray[Any, Any],
        dt_s: float,
    ) -> np.ndarray[Any, Any]:
        references = np.asarray(reference_points_enu, dtype=float) + (np.asarray(velocities_enu, dtype=float) * dt_s)
        positions = np.asarray(positions_enu, dtype=float)
        desired = np.asarray(desired_points_enu, dtype=float)

        for index in range(len(references)):
            distance_to_goal = float(np.linalg.norm(desired[index] - positions[index]))
            if distance_to_goal <= MOTION_REFERENCE_MAX_LEAD_M:
                references[index] = desired[index]
                continue

            lead = references[index] - positions[index]
            lead_norm = float(np.linalg.norm(lead))
            if lead_norm > MOTION_REFERENCE_MAX_LEAD_M:
                references[index] = positions[index] + (lead / lead_norm) * MOTION_REFERENCE_MAX_LEAD_M
        return references

    def _tracking_velocity_feedforward(
        self,
        positions_enu: np.ndarray[Any, Any],
        desired_points_enu: np.ndarray[Any, Any],
        velocities_enu: np.ndarray[Any, Any],
        ...
        positions = np.asarray(positions_enu, dtype=float)
        desired = np.asarray(desired_points_enu, dtype=float)
        velocities = np.asarray(velocities_enu, dtype=float).copy()
        distances = np.linalg.norm(desired - positions, axis=1)
        scale = np.clip(
            (distances - MOTION_FEEDFORWARD_STOP_RADIUS_M)
            / max(MOTION_FEEDFORWARD_FULL_RADIUS_M - MOTION_FEEDFORWARD_STOP_RADIUS_M, 1.0e-3),
            0.0,
            1.0,
        )
        return np.asarray(velocities * scale[:, np.newaxis], dtype=float)
    ...
    def _send_tracking_setpoints(
        ...
        feedforward = self._tracking_velocity_feedforward(positions_enu, desired_points_enu, velocities_enu)
        self._send_all_position_velocity_setpoints(desired_points_enu, feedforward)
    ...
    def _background_hold_loop(
        ...
        while self._running and not stop_event.is_set():
            ...
            if state_failure is not None:
                ...
                break
            positions = self._positions_enu()
            commands = self._behavior.velocity_commands(positions, target_points_enu, active)
    ...
    def _run_to_points(
        ...
        selected = selected_indices or set(range(len(self._drones)))
        ...
        while time.time() - motion_start_time < timeout_s:
            ...
            commands = self._behavior.velocity_commands(positions, desired, active)
            self._send_tracking_setpoints(positions, desired, commands)
            self._publish_fleet_status()

            errors = [float(np.linalg.norm(positions[index] - target_points_enu[index])) for index in selected]
            if errors:
                final_error = max(errors)
            if errors and max(errors) <= radius_m:
                dwell_elapsed += period
                if dwell_elapsed >= dwell_s:
                    self._command_all_for(np.zeros((len(self._drones), 3)), seconds=0.3)
                    return True, time.time() - command_start_time, max(errors)
            else:
                dwell_elapsed = 0.0
    ...
    def _run_waypoint_sequences(
        ...
        while time.time() - motion_start_time < timeout_s:
            ...
            if complete_count == len(self._drones):
                self._command_all_for(np.zeros((len(self._drones), 3)), seconds=0.3)
                return True, time.time() - command_start_time, self._sequence_progress(sequence_indices)
    ...
    def _run_to_shared_center(
        ...
        while time.time() - motion_start_time < timeout_s:
            ...
            final_error = float(np.max(center_errors)) if len(center_errors) else float("inf")
            if final_error <= complete_radius:
                dwell_elapsed += period
                if dwell_elapsed >= dwell_s:
                    self._command_all_for(np.zeros((len(self._drones), 3)), seconds=0.3)
    ...
    def _selected_target_points(
        ...
        for offset_index, drone_index in enumerate(selected_sorted):
            angle = (2.0 * math.pi * offset_index) / len(selected_sorted)
            points[drone_index] = center + np.array(
                [
    ...
    def _line_points(self, center_x: float, center_y: float, altitude: float, spacing_m: float) -> np.ndarray[Any, Any]:
    ...
    def _parse_points_json(self, points_json: str, altitude: float) -> np.ndarray[Any, Any]:
        ...
        points = self._positions_enu()
        for index, item in enumerate(parsed[: len(self._drones)]):
            points[index] = self._point_from_json_item(item, altitude)
        return points
    ...
    def takeoff_swarm(self, altitude: float = DEFAULT_TAKEOFF_ALTITUDE_M) -> str:
        ...
        def run() -> str:
            ...
            targets[:, 2] = max(altitude, 1.0)
            complete, duration_s, final_error = self._run_to_points(
                targets,
                radius_m=0.8,
                dwell_s=0.8,
                timeout_s=max(25.0, altitude * 8.0),
            )
            self._home_positions = self._positions_enu()
            ...
            return (
                ...
                f"final_error={final_error:.2f}m{hold}"
            )
    ...
    def return_to_launch(self, altitude: float = DEFAULT_TASK_ALTITUDE_M, land: bool = True) -> str:
        ...
        def run() -> str:
            ...
            if land:
                ...
                self._start_hold_after_motion(targets, reached_target=complete)
            status = "complete" if complete else "timed out"
            landing = " and land sent" if land else ""
            return (
    ...
    def go_to_points(
        ...
        Args:
            points_json: JSON list of points. A single point sends all drones around that center.
            altitude: Default z value when a point omits altitude.
            formation_radius: Radius around a single shared point.
        """
        ...
        def run() -> str:
            ...
            if len(parsed) == 1:
                center = self._point_from_json_item(parsed[0], altitude)
                ...
                targets = self._parse_points_json(points_json, altitude)
                selected = set(range(min(len(parsed), len(self._drones))))
                complete, duration_s, final_error = self._run_to_points(
                    targets,
            ...
            self._start_hold_after_motion(targets, reached_target=complete)
            status = "complete" if complete else "timed out"
            return f"go_to_points {status}: duration={duration_s:.1f}s, final_error={final_error:.2f}m"
    ...
    def go_to_shared_point(
        ...
        """
        points_json = json.dumps([{"x": x, "y": y, "z": z}])
        return self.go_to_points(points_json=points_json, formation_radius=formation_radius)
    ...
    def investigate_relative(
        ...
        Args:
            ...
            units: Number of drones to task.
